...
{
    CGFloat screenScale = 1;/*[UIScreen mainScreen].scale 为了配合滤镜使用，暂时设为1*/
    CGSize imageSize = image.size;
...
// 判断是否超过给定的比例
+ (BOOL)isLongwidePhoto:(UIImage*)image scale:(float)scale
{
    if (image) {
        CGFloat oWidth = image.size.width;
        CGFloat oHeight = image.size.height;
        if ((oWidth / oHeight) >= scale || (oHeight / oWidth) >= scale)
            return YES;
        else
            return NO;
    }
